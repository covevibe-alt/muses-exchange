...
// Slug used in URL: lowercase artist name with hyphens. SEO-friendlier
// than ticker — "drake" ranks for "drake" queries; "drke" doesn't.
// Strips diacritics ("Rosalía" → "rosalia") and collapses non-alphanum
// to single hyphens. Verified zero collisions across the 105 roster.
...
<style>
  ...
  /* External / inline content links — match the muted purple accent
     instead of the browser-default bright blue (which clashed with
     the dark theme). Underline removed by default, surfaces on hover. */
  .artist-content a {
    color: #b98fff;
    text-decoration: none;
    border-bottom: 1px solid rgba(185, 143, 255, 0.30);
    transition: border-color .15s ease, color .15s ease;
  }
  .artist-content a:hover {
    color: #d4b8ff;
    border-bottom-color: rgba(185, 143, 255, 0.70);
  }
  .artist-content ul {
    margin: 0 0 16px;
    padding-left: 22px;
  }
  .artist-content ul li { margin-bottom: 6px; }
  /* Trade-CTA card — replaces the old inline text link with a more
     prominent action under the "Start trading $X" heading. */
  .artist-cta-box {
  ...
  .artist-cta-button { flex-shrink: 0; }
  .artist-related {
...
<section class="artist-content">
  ...
  <div class="artist-cta-box">
    <div class="artist-cta-text">
      <div class="artist-cta-eyebrow">Ready to trade?</div>
      <div class="artist-cta-title">Open ${escapeHtml(name)}'s trading page</div>
    </div>
    <a class="btn-primary artist-cta-button" href="/exchange/${escapeHtml(ticker.toLowerCase())}">Trade $${escapeHtml(ticker)} <svg><use href="#i-arrow-up-right"/></svg></a>
  </div>
</section>
...
<script>
  ...
  (function () {
    ...
    // Port of the SPA's computeFairPrice() so the displayed price matches
    // the trading view's $100–500 range instead of the raw LIVE_MARKET.price
    // ($1–5 range). PRICING constants mirror exchange.html exactly.
    const PRICING = {
    ...
    const computeFairPrice = (artist, allArtists) => {
      const listeners = (artist && artist.monthlyListeners) || 0;
      const total = allArtists.reduce((s, x) => s + (x.monthlyListeners || 0), 0);
      const totalCap = Math.max(PRICING.baseMarketCap, allArtists.length * PRICING.capPerArtist);
      let marketCap;
      if (total > 0) marketCap = (listeners / total) * totalCap;
      else marketCap = listeners * PRICING.valuePerListener;
      const base = Math.max(0.01, marketCap / PRICING.sharesOutstanding);
      const yt = artist.youtubeBoost || 0;
      const ch = artist.chartBoost || 0;
      const pop = artist.popularityBoost || 0;
      // listenerMomentum is computed from historyPoints which aren't in
      // LIVE_MARKET — we accept a few % approximation vs the SPA.
      return base * (1 + yt + ch + pop);
    };
    const apply = (data) => {
      ...
      // Compute the SPA-equivalent fair price instead of using a.price
      // (which is the raw market-index price from prices.py — different
      // formula, off by ~50x).
      const displayPrice = computeFairPrice(a, data.artists);
      setText('[data-stat="price"]', fmtPrice(displayPrice));
      if (a.monthlyListeners != null) setText('[data-stat="listeners"]', fmtListeners(a.monthlyListeners));
      if (a.chg24h != null) setText('[data-stat="change"]', fmtChange(a.chg24h));
      return true;
    };
    // prices.js exposes window.LIVE_MARKET (verified live). First pass:
    // poll briefly for the deferred script to load and apply on first hit.
    if (!apply(window.LIVE_MARKET)) {
      let tries = 0;
      const t = setInterval(() => {
        tries++;
        if (apply(window.LIVE_MARKET) || tries > 40) clearInterval(t);
      }, 250);
    }
    // Real-time freshness: re-fetch /app/prices.js every 5 minutes so the
    // page stays in sync with the cron-driven price updates without
    // requiring a manual reload. Cache-bust with a timestamp param.
    setInterval(async () => {
      try {
        const res = await fetch('/app/prices.js?_t=' + Date.now(), { cache: 'no-store' });
        if (!res.ok) return;
        const code = await res.text();
        // prices.js sets window.LIVE_MARKET = {...}; eval in a sandboxed
        // function so we don't pollute global scope beyond LIVE_MARKET.
        // Trust this source — it's same-origin and only contains a JSON
        // assignment.
        new Function(code)();
        apply(window.LIVE_MARKET);
      } catch (_) {}
    }, 5 * 60 * 1000);
  })();
</script>
...
function build() {
  ...
  if (!fs.existsSync(OUT_DIR)) {
    ...
    // Clean out old .html files first — when slugs change (e.g. ticker→
    // name migration), stale files would otherwise sit around as duplicate
    // content. Only deletes .html so non-html assets (if any) stay.
    let cleaned = 0;
    ...
    if (cleaned > 0) console.log(`Cleaned ${cleaned} old .html files`);
  }
